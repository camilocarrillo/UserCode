...
/afs/cern.ch/user/c/carrillo/public/for_All/cafDynamic/checkRunsInDataSet.sh $1 $dataset
...
do
	...
	echo "Calling simpleSpecifyPathFAST for run $run (see /tmp/$myuser/$label-$run.txt)"
	/afs/cern.ch/user/c/carrillo/public/for_All/cafDynamic/simpleSpecifyPathFast.sh $label-$run.txt 1> /tmp/$myuser/$label-$run.txt &
	let "i=$i+1"
done
