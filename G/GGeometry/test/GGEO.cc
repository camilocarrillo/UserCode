...
// Class:      GGEO
// 
/**\class GGEO GGEO.cc rpcgeo/GGEO/src/GGEO.cc
...
// $Id: GGEO.cc,v 1.8 2011/10/18 13:23:18 yana Exp $
//
//
...
class GGEO : public edm::EDAnalyzer {
   public:
      explicit GGEO(const edm::ParameterSet&);
      ~GGEO();
...
//
GGEO::GGEO(const edm::ParameterSet& /*iConfig*/){
...
void
GGEO::analyze(const edm::Event& /*iEvent*/, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for (TrackingGeometry::DetContainer::const_iterator it=rpcGeo->dets().begin();it<rpcGeo->dets().end();it++){
	...
	//std::cout<<rpcsrv.name()<<std::endl;
	 ...
	 if (rpcId.region()==0){ 
	   //std::cout<<"Getting the G Topolgy"<<std::endl;
