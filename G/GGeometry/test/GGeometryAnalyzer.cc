...
class GGeometryAnalyzer : public edm::EDAnalyzer {

 public: 
  GGeometryAnalyzer( const edm::ParameterSet& pset);
...
GGeometryAnalyzer::GGeometryAnalyzer( const edm::ParameterSet& /*iConfig*/ )
  : dashedLineWidth_(104), dashedLine_( std::string(dashedLineWidth_, '-') ), 
    myName_( "GGeometryAnalyzer" ) 
{ 
...
GGeometryAnalyzer::~GGeometryAnalyzer() 
{
  ofos.close();
...
void
GGeometryAnalyzer::analyze( const edm::Event& /*iEvent*/, const edm::EventSetup& iSetup )
{
...
//      //----------------------- GCHAMBER TEST -------------------------------------------------------
      ...
      if( dynamic_cast< GChamber* >( *it ) != 0 ){
       ...
       //       const GRoll* rollRaf = ch->roll(1);
       std::cout<<"Num = "<<iGCHcount<<"  "<<"GDet = "<<idRaf<<"  Num Rolls =" <<ch->nrolls()<<std::endl;
	 //       "  "<<"Roll 1 = "<<(rollRaf->id()).rawId()<<std::endl;

       std::vector< const GRoll*> rollsRaf = (ch->rolls());
       for(std::vector<const GRoll*>::iterator r = rollsRaf.begin();
	   r != rollsRaf.end(); ++r){

	 if((*r)->id().region() == 0){
	 std::cout<<"GDetId = "<<(*r)->id().rawId()<<std::endl;
...
//      if( dynamic_cast< GRoll* >( *it ) != 0 ){
//        ++icount;

//        GRoll* roll = dynamic_cast< GRoll*>( *it );

//        GDetId detId=roll->id();
...
DEFINE_FWK_MODULE(GGeometryAnalyzer);
