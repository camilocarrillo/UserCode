...
void GraphAndFit(){
  ...
  TFile *_file0 = TFile::Open("9000/sec109815.root");
  TFile *_file1 = TFile::Open("9100/sec109718.root");
  TFile *_file2 = TFile::Open("9200/sec109468.root");
  TFile *_file3 = TFile::Open("9300/sec109578.root");
  ...
  while ( !filein.eof()){
    ...
    f1 = new TF1("f1",fitfunc,8.5,9.5, 3 ); //antes era 3 ojo!!!
    ...
    if(HV9000&&HV9100&&HV9200&&HV9300){

      efftxt9000 >> roll[0] >> avEFF[0] >> avERR[0] >> mask[0];
      efftxt9100 >> roll[1] >> avEFF[1] >> avERR[1] >> mask[1];
      efftxt9200 >> roll[2] >> avEFF[2] >> avERR[2] >> mask[2];
      efftxt9300 >> roll[3] >> avEFF[3] >> avERR[3] >> mask[3];
      
      if(! (roll[0].compare(roll[1]) && roll[1].compare(roll[2]) && roll[2].compare(roll[3]) && roll[3].compare(roll[0]))){
	cout<<"All strings Match!!"<<endl;
      }else{
	cout<<"!!!!!!!!!!!!!!! STOP bad matching files !!!!!!!!!!!!"<<endl;
	for(int i=0;i<5;i++) cout<<roll[i]<<" "; cout <<Roll<<endl;
	cout<<roll[0].compare(roll[1]) << roll[1].compare(roll[2]) << roll[2].compare(roll[0])<<endl;
	break;
      }
      
      x[0] = 9.0; //correction for pressure and temperature here
      x[1] = 9.1; //correction for pressure and temperature here
      x[2] = 9.2; //correction for pressure and temperature here
      x[3] = 9.3; //correction for pressure and temperature here
