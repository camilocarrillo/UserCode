...
for line in `head -n $max files`
	...
	echo $padROOT
	echo $rootfile
	sed -e "s|-filename-|$rootfile|g" -e "s|-pad-|$padROOT|g" $cfgtemplate > $rootfile.cfg
	sed -e "s|-filename-|$rootfile|g" -e "s|-pad-|$padROOT|g" -e "s|-padtorun-|$padRUN|g" -e "s|-currentpad-|$padCURR|g" -e "s|-storepad-|$storePAD|g" -e "s|-key-|$key|g" ~carrillo/public/for_All/cafDynamic/jobtemplateDynamic > $rootfile.lsf
